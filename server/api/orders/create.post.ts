...
export default defineEventHandler(async (event) => {
  ...
  if (turnstileSecret) {
    ...
    if (!verified) {
      ...
      throw createError({ statusCode: 403, message: 'Verificación de seguridad inválida. Recarga e inténtalo de nuevo.' })
  ...
  const stockReservations = await reserveCartStock(cartItems)
  ...
  try {
    ...
    await releaseCartStockReservations(stockReservations)
  ...
  if (idempotencyKey) {
    ...
    const saved = await saveIdempotentOrder(idempotencyKey, subject.cartKey, response)
