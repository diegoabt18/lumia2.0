<template>
  <div>
    <ul class="space-y-3" :class="compact ? 'pr-1' : ''">
      <li v-for="item in items" :key="item.sku" class="flex items-start gap-3">
        ...
        <div class="min-w-0 flex-1">
          <NuxtLink :to="`/products/${item.productSlug}`" class="line-clamp-2 text-sm font-medium leading-snug text-lumia-ink hover:text-lumia-gold">
            {{ item.productName }}
          </NuxtLink>
